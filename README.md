...
> 一张照片，孵化一个会陪你写代码的桌面 AI 搭子。

把你家猫猫、狗狗或其他宠物的照片，制作成会待机、奔跑、挥爪、跳跃、等待、工作、审查，并能跟随光标环视的 Codex Desktop v2 动画宠物。无需手工切 88 个格子，Skill 会带你完成从照片分析到安装验收的完整流程。

[立即安装](#安装-skill) · [查看使用方法](#最简单的使用方式) · [常见问题](#常见问题) · [宣传素材](PROMOTION.md)

如果你也想让自家宠物住进 Codex，欢迎 [Star 本项目](https://github.com/chenbaiji518-oss/codex-cyber-pet)、分享给朋友，或提交 Issue 晒出你的赛博萌宠。

这不是一份只教你“生成一张萌宠图”的提示词，而是一套可复用 Skill：从照片筛选、角色特征锁定、动作生成、8×11 图集组装，到透明度检查、安全边距修复、本地安装和缓存排错，形成完整闭环。

## 为什么值得试试

- **它长得像你的宠物**：锁定眼睛、脸型、毛色、纹路和尾巴等身份特征
- **它不只会站着**：包含 9 组标准动作和 16 个光标环视方向
- **它真的能装进 Codex**：自动组装并验证 Codex Desktop v2 图集
- **踩过的坑都写进去了**：覆盖默认机器人未替换、缓存不刷新、脚和尾巴被裁切等问题
- **完全开源**：Skill、验证脚本、安装脚本和中文说明均可自由使用与改进
